...
async function scrapeCategories() {
  try {
    ...
    // JSPATH to identify the correct elements for subjects
  ...
  for (const subject in data) {
    for (const topic in data[subject].Topics) {
      ...
      try {
        ...
        const $ = cheerio.load(response.data);
        $('#taxonomy-term-9358 > div > div > div > div.block.block-revision-study-resources.block-revision-study-resources-section-child-links > div > ul > li > a').each((i, subtopicElement) => {
